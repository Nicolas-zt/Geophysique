...
#Estimation du post-sismique à partir du signal corrigé du cosismique
def MC_postsismic_1(Station, coord, date_seisme, date_fin, sol, plot=False):
    #Pour récupérer les écart-types dans la bonne colonne
    table_ecart_type = {"dN":"Sn", "dE":"Se", "dU":"Su"}
    ecart_type = table_ecart_type[coord]
    
    station = Station.copy()
    station = station[station["datetime"] > date_seisme]
    station = station[station["datetime"] < date_fin]
    
    A = np.zeros((station.shape[0], 2))
    P = np.identity(station.shape[0])
    B = np.zeros((station.shape[0], 1))
    ...
    for k in range(10):
        # Extraction des paramètres non-linéaires courants
        A0 = sol[0,0]   # amplitudes Ak
        a0 = sol[1,0]   # taux de relaxation ak
        ...
        for i in range(station.shape[0]):
            ...
            A[i, :] = (
                # ∂/∂Ak = exp(-ak * dt)
                [np.exp(-a0 * (t - date_seisme)) * H(t - date_seisme)]
                # ∂/∂ak = -Ak * dt * exp(-ak * dt)
                + [-A0 * (t - date_seisme) * np.exp(-a0 * (t - date_seisme)) * H(t - date_seisme)]
            )

            P[i, i] = 1.0 / ligne[ecart_type]**2
            B[i, 0] = ligne[coord + "_cosismic"] - f(t, sol)
    ...
    if plot:
        fig, ax = plt.subplots()
        l = [f(station.iloc[k]["datetime"], sol) for k in range(station.shape[0])]
        t = station["datetime"]
        ax.plot(t, station[coord + "_cosismic"], label="données")
        ax.plot(t, l, label="modèle")
        ax.legend()
        plt.show()
        
    Amplitude_t0 = A0*np.exp(0)
    Amplitude_t = A0*np.exp(-a0*5)
...
#%%
    ...
    lons   = [lon_CNBA, lon_DINO, lon_UCOR]
    lats   = [lat_CNBA, lat_DINO, lat_UCOR]
    labels = ["CNBA", "DINO", "UCOR"]
    ...
    q = ax.quiver([lon_CNBA, lon_DINO, lon_UCOR],
              [lat_CNBA, lat_DINO, lat_UCOR],
              [CNBA_Long_terme[0]*10, DINO_Long_terme[0]*10, UCOR_Long_terme[0]*10],
              [CNBA_Long_terme[1]*10, DINO_Long_terme[1]*10, UCOR_Long_terme[1]*10],
              transform=ccrs.PlateCarree(),
              scale=0.1,
              scale_units='inches')
    
    ax.quiverkey(q, X=1.2, Y=0.5, U=0.1, label='0.01 m/an',
             labelpos='E', coordinates='axes')
    ax.set_title("Vitesse long terme")
    ...
    CosismicE_DINO = MC_cosismic(DINO, "dE",[2016.9644],[2015+259/365.25,2014+91/365.25])
    print(f"Amplitude Cosismique Est Iquique sur DINO : {round(CosismicE_DINO[1,0],3)} m")
    CO_E_DINO = round(CosismicE_DINO[1,0],3)
    
    CosismicN_DINO = MC_cosismic(DINO, "dN",[2016.9644],[2015+259/365.25,2014+91/365.25])
    print(f"Amplitude Cosismique Nord Iquique sur DINO : {round(CosismicN_DINO[1,0],3)} m")
    CO_N_DINO = round(CosismicN_DINO[1,0],3)
    
    CosismicE_UCOR = MC_cosismic(UCOR, "dE",[2008.8767,2017.6438,2019.4137],[2010+58/365.25,2015+259/365.25,2014+91/365.25])
    print(f"Amplitude Cosismique Est Iquique sur UCOR : {round(CosismicE_UCOR[4,0],3)} m")
    CO_E_UCOR = round(CosismicE_UCOR[4,0],3)
    
    CosismicN_UCOR = MC_cosismic(UCOR, "dN",[2008.8767,2017.6438,2019.4137],[2010+58/365.25,2015+259/365.25,2014+91/365.25])
    ...
    for lon, lat, label in zip(lons, lats, labels):
        ax.text(lon, lat, label, transform=ccrs.PlateCarree())
    ax.scatter([lon_CNBA,lon_DINO,lon_UCOR],[lat_CNBA,lat_DINO,lat_UCOR],color = 'green', transform=ccrs.PlateCarree())
    q = ax.quiver([lon_CNBA,lon_DINO,lon_UCOR],[lat_CNBA,lat_DINO,lat_UCOR],
              [CO_E_CNBA*5,CO_E_DINO*5,CO_E_UCOR*5],
              [CO_N_CNBA*5,CO_N_DINO*5,CO_N_UCOR*5],
              transform=ccrs.PlateCarree(),
              scale=5,

              scale_units='inches')
    ax.quiverkey(q, X=1.2, Y=0.5, U=1, label='1m', labelpos='E')
    ax.set_title("Déplacement co-sismique")
    
    """#Estimation_CNBA = MC_postsismic(CNBA, "dE",[],[2015+259/365.25],np.array([0]+[1.]+[1.]),True)
    Estimation_CNBA = MC_postsismic(CNBA, "dN",[],[2010+58/365.25,2015+259/365.25],np.array([0,0]+[1.,1.]+[1.,1.]),True)
    Estimation_DINO = MC_postsismic(DINO, "dE",[2016.9644],[2015+259/365.25],np.array([0,0]+[0.1]+[1]), True)
    Estimation_DINO = MC_postsismic(DINO, "dN",[2016.9644],[2015+259/365.25],np.array([0,0]+[0.1]+[1]), True)
    Estimation_UCOR = MC_postsismic(UCOR, "dE",[2008.8767],[2010+58/365.25,2015+259/365.25],np.array([0,0,0]+[0.1,0.1]+[1,1]), True)
    #Estimation_UCOR = MC_postsismic(UCOR, "dN",[2008.8767,2017.6438],[2010+58/365.25,2015+259/365.25],np.array([0,0,0,0]+[0.1,0.1]+[1,1]), True)"""
    
    POST_CNBA_E = MC_postsismic_1(CNBA, "dN",2015+259/365.25,2020,np.array([[1.,1.]]).T)
    POST_CNBA_N = MC_postsismic_1(CNBA, "dE",2015+259/365.25,2020,np.array([[1.,1.]]).T)
    POST_DINO_E = MC_postsismic_1(DINO, "dN",2015+259/365.25,2020,np.array([[1.,1.]]).T)
    POST_DINO_N = MC_postsismic_1(DINO, "dE",2015+259/365.25,2016.86,np.array([[1.,1.]]).T)
    POST_UCOR_E = MC_postsismic_1(UCOR, "dN",2015+259/365.25,2017.6438,np.array([[1.,1.]]).T)
    POST_UCOR_N = MC_postsismic_1(UCOR, "dE",2015+259/365.25,2017.6438,np.array([[1.,1.]]).T)
    
    print(POST_CNBA_E,POST_DINO_E,POST_UCOR_E)
    print(POST_CNBA_N,POST_DINO_N,POST_UCOR_N)
    ...
    ax.scatter([lon_CNBA,lon_DINO,lon_UCOR],[lat_CNBA,lat_DINO,lat_UCOR],color = 'green', transform=ccrs.PlateCarree())
    q = ax.quiver([lon_CNBA,lon_DINO,lon_UCOR],[lat_CNBA,lat_DINO,lat_UCOR],
              [POST_CNBA_E*10,POST_DINO_E*10,POST_UCOR_E*10],
              [POST_CNBA_N*10,POST_DINO_N*10,POST_UCOR_N*10],
              transform=ccrs.PlateCarree(),
              scale=1,
              scale_units='inches')
    
    ax.quiverkey(q, X=1.2, Y=0.5, U=1, label='0.1m', labelpos='E')
    ax.set_title("Déplacement post-sismique")